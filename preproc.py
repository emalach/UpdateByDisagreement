...
tf.app.flags.DEFINE_float('label_noise_rate', 0, 'Rate of label noise to be injected into the data')
...
def _process_dataset(name, filename, directory, num_shards):
    ...
    # Inject label noise by given rate
    # TODO: make this more general...
    if name == 'train':
        for i in range(len(labels)):
            if random.random() < FLAGS.label_noise_rate:
                print('flipping')
